...
const room = "columbus-buffalo-pea"; // Nom de la salle de jeu
...
function cleanCards(cards: string[]) {
...
async function generateClue(teamCards: string[], enemyCards: string[], neutralCards: string[], blackCard: string) {
...
async function checkAndGenerateClue(page: any) {
    ...
    if (await clueInput.isVisible()) {
        ...
        const redCards = await page.locator('.card.red').allTextContents();  // Cartes de l'équipe rouge
        const blueCards = await page.locator('.card.blue').allTextContents();  // Cartes de l'équipe bleue
        const neutralCards = await page.locator('.card.gray').allTextContents();  // Cartes neutres
        // 📌 Récupérer la carte noire (on prend le premier élément de la liste retournée)
        const blackCard = (await page.locator('.card.black').first().allTextContents()).join("");
        ...
        const foundCards = await page.evaluate(() => {
        ...
        const { clue, numberOfWords } = await generateClue(remainingRedCards, remainingBlueCards, remainingNeutralCards, blackCard);
        ...
        await numberSelectButton.click();
...
(async () => {
    ...
    await page.goto("https://codenames.game/room/" + room);
    ...
    await pseudoInput.fill("Marie BOT");
    ...
    while (true) {
        await checkAndGenerateClue(page);
        await page.waitForTimeout(15000); // Attendre 15 secondes avant de vérifier à nouveau
    }
})();